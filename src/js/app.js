...
async function getArr() {
    ...
    const shortInfo = document.querySelector('.short-info'),
        modalPrice = document.createElement('span'),
        modalNameCard = document.createElement('span');

    modalPrice.className = 'modal-price';
    modalNameCard.className = 'modal-name-card';


    shortInfo.append(modalNameCard, modalPrice);
        ...
        el.addEventListener('click', (e) => {
            ...
            const targetImg = cards.find((item) => {
                if ('key' + item.id === imgId) {
                    ...
                    modalNameCard.innerText = item.nameCard;
                };
            });

            modalVisible.prepend(bigImg);
    ...
    btnClose.addEventListener('click', (X) => {
        if (X.target == btnClose) {
